 ...
 *
 *  Compilers used: SAS/C 6.58 / VASM 1.9f 
 *
 *  IMPORTANT NOTE:
 *  FOR THE PRODUCTIVE, FINAL VERSION VASM 1.9a WAS USED TO COMPILE THE FPU
 *  DETECTION CODE (THE OPCODES HAVE THEN BEEN INCLUDED AS HEX VALUES, SO THAT
 *  COMPILATION WITH OTHER VERSIONS SHOULD NOT ALTER THIS CODE). THE VASM 1.9a
 *  OPCODES SEEM TO BE "INOFFESIVE" TO CHECK THE ABILITY OF THE FPU ON V2 TO DO 
 *  3-OPERANT INSTRUCTIONS). FOR THE FINAL COMPILATION VASM 1.9f HAS BEEN USED
 *  (BINARIES GENERATED BY VASM 2.0beta SEEMED UNSTABLE).
 *
 *  V2 USERS SHOULD WAIT FOR THE UPCOMING GOLD 2.18 CORE RELEASE TO USE 
 *  3-OPERANT INSTRUCTIONS (THE OPTIMIZED VAMPIRE CODE WILL NOT WORK ON 
 *  CORES <= 2.17).
 *
...
void SystemSpecificMessageAndValues(void)
{
  if (detected_system==RUNNING_ON_AROS)
  {
     ...
     */	
    if (FM_APOLLOOS) FM_ALLOCTEMPRAST_FLAGS=BMF_INTERLEAVED | BMF_CLEAR | BMF_MINPLANES; /* BMF_INTERLEAVED works on ApolloOS >20240901 */
    else FM_ALLOCTEMPRAST_FLAGS=BMF_CLEAR | BMF_MINPLANES; /* BMF_INTERLEAVED doesn´t work on AROS */
	...
	if (IconToolTypes = (CONST_STRPTR *) ArgArrayInit (Argc,Argv))
  	{
        ...
        if ( 
        	strcmp( ArgString (IconToolTypes,"SYSTEM",""), "AROS"
           		...
           		) 
                {
                	detected_system=RUNNING_ON_AROS;
                    FM_APOLLOOS=TRUE;
                    /* interleaved bitmaps has been fixed 20240901 on ApolloOS 
                     * (when using an earlier rom, select AROS for ApolloOS
                     * to avoid "memory freed twice" bug)
                     */
                	FM_ALLOCTEMPRAST_FLAGS=BMF_INTERLEAVED | BMF_CLEAR | BMF_MINPLANES;
                }
        else if (
        ...
        /* enable/disable FPU check on V2 */		
        if ( 
        	strcmp( ArgString (IconToolTypes,"FPUCHECK",""), "YES"
            	  )
                  == NULL
           ) FM_FPUCHECK=TRUE;
        else if (
        		 strcmp( ArgString (IconToolTypes,"FPUCHECK",""), "NO"
            	       )
                  	   == NULL
           		) FM_FPUCHECK=FALSE;
